"""CKAN Open Data API client for the ckan module.

Provides async functions for fetching, shaping, caching, and rate-limiting
all CKAN API endpoints. All public functions return (data, was_cached) tuples.
...
from mcp_canada.modules.ckan.constants import (
    BASE_URL,
    CACHE_TTL_META,
    CACHE_TTL_SEARCH,
...
def _shape_resource(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        raw: Raw resource dict from CKAN API.
...
def _shape_dataset(raw: dict[str, Any], lang: str = "en") -> dict[str, Any]:
    """Shape a raw CKAN dataset dict for token-efficient agent consumption.
    ...
    Args:
        raw: Raw dataset dict from CKAN package_show or package_search result.
...
def _build_cache_key(path: str, params: dict[str, Any]) -> str:
    ...
    Returns:
        Cache key string with 'ckan:' prefix.
    """
    sorted_params = "&".join(f"{k}={v}" for k, v in sorted(params.items()))
    return f"ckan:{path}?{sorted_params}"
...
async def _api_get(
    ...
    """Internal helper: fetch from CKAN API with caching, rate limiting, and envelope unwrapping.
...
async def fetch_search_datasets(
    ...
    """Search CKAN datasets by keyword with optional filter query.

    Args:
        query: Solr search query string (q param).
        fq: Optional filter query (e.g. 'tags:water' or 'organization:gc').
        rows: Number of results to return.
        start: Offset for pagination.
        sort: Sort order string.
        lang: Language for shaping results.
...
async def fetch_dataset_details(
    ...
    """Fetch full details for a single dataset, shaped and truncated.

    Args:
        dataset_id: CKAN dataset ID or name slug.
        lang: Language for bilingual field extraction.
...
async def fetch_organizations(
    ...
    """Fetch list of CKAN organizations.
...
async def fetch_groups(
    ...
    """Fetch list of CKAN thematic groups.

    Args:
        all_fields: If True, include full group details.

    Returns:
        (list of group dicts, was_cached)
    """
    params: dict[str, Any] = {"all_fields": all_fields}
    result, was_cached = await _api_get("action/group_list", params, CACHE_TTL_META)
    return result, was_cached
...
async def fetch_resource(
    ...
    """Fetch details for a specific resource by ID.
...
async def fetch_dataset_count() -> tuple[int, bool]:
    """Fetch total number of datasets in the CKAN portal.
