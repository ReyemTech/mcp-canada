...
async def nb_crown_land_report(
    ...
    if lang == "fr":
        return [
            ...
            Message(
                ...
                "la foresterie du Nouveau-Brunswick en quatre étapes.\n\n"
                "**D'abord, ce que le paramètre `county` peut et ne peut pas délimiter.** La "
                "couche 3 des terres de la Couronne ne contient que "
                "`OBJECTID`/`Shape`/`HOLDER`/`Shape_Length`/`Shape_Area`, et la couche des "
                "occurrences minérales seulement `URN`/`LAT`/`LON`/`NAME`/`COMMODITIE` — "
                "**aucune de ces couches ne possède d'attribut de comté**, donc aucune ne "
                "peut être filtrée par comté. Les étapes 2 et 3 ci-dessous couvrent toute la "
                "province ; indiquez-le dans le rapport plutôt que de laisser croire le "
                "contraire.\n\n"
                "**Étape 1 — Terres délimitées par comté (c'est ici que le comté s'applique "
                "réellement) :**\n"
                "Appelez `nb_get_parcels` avec `county=` pour obtenir les parcelles du comté "
                "demandé — c'est la seule couche foncière filtrable par comté dans ce module. "
                "Utilisez-la pour cadrer la portée géographique du rapport, puis traitez les "
                "résultats ci-dessous comme un contexte provincial.\n\n"
                "**Étape 2 — Parcelles de terres de la Couronne (toute la province) :**\n"
                ...
                "**Étape 3 — Découvrir la couche des occurrences minérales :**\n"
                ...
                "**Étape 4 — Interroger les occurrences minérales et la foresterie (longue "
                "traîne, toute la province) :**\n"
    ...
    return [
        ...
        Message(
            ...
            "four steps.\n\n"
            "**First, what `county` can and cannot scope.** Crown Land layer 3 carries only "
            "`OBJECTID`/`Shape`/`HOLDER`/`Shape_Length`/`Shape_Area`, and the mineral "
            "occurrences layer only `URN`/`LAT`/`LON`/`NAME`/`COMMODITIE` — **neither layer "
            "has a county attribute**, so neither can be filtered by county. Steps 2 and 3 "
            "below are province-wide; say so in the report rather than implying otherwise.\n\n"
            "**Step 1 — County-scoped land (this is where the county actually applies):**\n"
            "Call `nb_get_parcels` with `county=` to get the parcels for the requested "
            "county — this is the only county-filterable land layer in the module. Use it to "
            "frame the report's geographic scope, then treat the Crown land and mineral "
            "results below as provincial context.\n\n"
            "**Step 2 — Crown land parcels (province-wide):**\n"
            ...
            "name.\n\n"
            "**Step 3 — Discover the mineral occurrences layer:**\n"
            ...
            "guessable.\n\n"
            "**Step 4 — Query mineral occurrences and forestry (long tail, province-wide):**\n"
