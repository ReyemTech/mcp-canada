"""Edmonton module client — async functions returning (data, was_cached) tuples.
...
# Default is None (keyless). Set EDMONTON_APP_TOKEN env var for higher throttle limits.
APP_TOKEN: str | None = os.environ.get(EDMONTON_APP_TOKEN_ENV)

# Single shared rate limiter for all Edmonton SODA calls.
...
async def fetch_search_datasets(
    ...
    """Search the Edmonton Socrata catalog by keyword with pagination.
...
async def fetch_query_dataset(
    ...
    """Pass-through SoQL query for any Edmonton dataset.
...
async def fetch_organizations() -> tuple[dict[str, Any], bool]:
    """List unique organization attributions from the Edmonton Socrata catalog.
...
async def fetch_categories() -> tuple[dict[str, Any], bool]:
    """List domain categories from the Edmonton Socrata catalog.
    ...
    on data.novascotia.ca) and was never live-verified as working on Edmonton either,
