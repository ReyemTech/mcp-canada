...
"""Unit tests for Edmonton module prompts.py and resources.py."""
...
class TestEdmontonPrompts:
    @pytest.mark.asyncio
    async def test_explore_open_data_returns_list_of_messages(self) -> None:
        from mcp_canada.modules.edmonton.prompts import edmonton_explore_open_data

        result = await edmonton_explore_open_data(lang="en")
    ...
    async def test_explore_open_data_references_tools(self) -> None:
        ...
        assert "edmonton_search_datasets" in text
        assert "edmonton_get_dataset_details" in text
        assert "edmonton_query_dataset" in text

    @pytest.mark.asyncio
    async def test_explore_open_data_bilingual(self) -> None:
        from mcp_canada.modules.edmonton.prompts import edmonton_explore_open_data

        en = await edmonton_explore_open_data(lang="en")
        fr = await edmonton_explore_open_data(lang="fr")
    ...
    async def test_quick_find_dataset_returns_str(self) -> None:
        from mcp_canada.modules.edmonton.prompts import edmonton_quick_find_dataset

        result = await edmonton_quick_find_dataset(lang="en")
    ...
    async def test_quick_find_dataset_mentions_tool(self) -> None:
        ...
        result = await edmonton_quick_find_dataset(lang="en")
        assert "edmonton_search_datasets" in result

    @pytest.mark.asyncio
    async def test_quick_find_dataset_bilingual(self) -> None:
        from mcp_canada.modules.edmonton.prompts import edmonton_quick_find_dataset

        en = await edmonton_quick_find_dataset(lang="en")
        fr = await edmonton_quick_find_dataset(lang="fr")
        assert en != fr


class TestEdmontonResources:
    @pytest.mark.asyncio
    async def test_portal_guide_returns_str(self) -> None:
        from mcp_canada.modules.edmonton.resources import edmonton_portal_guide

        result = await edmonton_portal_guide()
        assert isinstance(result, str)
