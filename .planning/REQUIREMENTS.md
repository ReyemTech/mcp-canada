...
- [x] **ERR-06**: Blame is declared at the raise site — `InvalidInput` / `NotFound` / `UpstreamData` from `shared/errors.py`; a bare `raise ValueError` is banned in source
- [x] **ERR-07**: A plain `ValueError` defaults to `UPSTREAM_ERROR`; `except ValueError -> INVALID_INPUT` is banned, so an unrecognised subclass can never be blamed on the caller
...
| ERR-06 | Phase 20.4 | Complete |
| ERR-07 | Phase 20.4 | Complete |
