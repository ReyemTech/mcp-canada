...
async def alberta_get_hospitals(
    ...
    Keywords: alberta hospitals AHS AHSGIS arcgis health facility inpatient emergency department zone address
    """
...
async def alberta_get_ahs_zones(
    ...
    Use for: AHS administrative zones — South, Calgary, Central, Edmonton, North. Each zone includes name, ID, and census population (2006/2011/2016). Pass include_geometry=true for zone boundary polygons. These are the zones referenced by every AHS facility record.
...
async def alberta_get_health_facilities(
    ...
    Use for: Non-hospital Alberta health facilities. facility_type='ems' returns EMS (Emergency Medical Services) stations; facility_type='pcn_clinic' returns PCN (Primary Care Network) clinics and walk-in clinics. For hospitals, use alberta_get_hospitals. For zones, use alberta_get_ahs_zones.

    Keywords: alberta health facilities EMS ambulance PCN primary care clinic walk-in AHS AHSGIS dispatch
    """
    raise NotImplementedError("Plan 05 implements")
