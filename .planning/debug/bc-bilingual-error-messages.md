...
fix: |
  **Recommended minimum-diff fix — localise only the water wells guard (Test 13 scope):**

  At `src/mcp_canada/modules/british_columbia/tools.py:700-706`, replace:
  ...
  **Rationale for the inline ternary over `t()`:**
  1. Zero production code currently imports `shared/i18n.py:t()`. Introducing the first import via a one-off guard message would be a larger, scope-creeping change.
  2. The inline `lang == "fr"` branch mirrors the established pattern used throughout `prompts.py` files across 7 modules (statcan, weather, bank_of_canada, drug_database, york_region, ircc, +BC's own `bc_explore_wildfires` prompt). It is the closest to an existing convention.
  3. The guard message is too specific ("Pitfall 5", a tool name, a 130K figure) to fit cleanly into a reusable i18n catalog key.
  4. Minimum diff: 6 lines changed in one function. No new imports. No new i18n keys. No risk of cascading into other modules. Test 13 goes green.

  **Regression test to add** alongside the fix (required by TDD rules in `.claude/rules/tests.md`):

  In `src/mcp_canada/modules/british_columbia/__tests__/test_tools.py`, add a test that calls `bc_get_water_wells(lang="fr")` with no filter and asserts the returned `error.message` contains `"au moins un"` (or another French-only substring) and `error.lang == "fr"`.

  **NOT recommended for this gap (scope creep):**
  - Migrating all ~50 tools across all modules to use `shared/i18n.py:t()`.
  - Adding a new `error.bc_water_wells_guard` key to `shared/i18n.py:LABELS`.
  - Making `make_error` translate messages automatically (would change the shared contract and touch every module).

  Those are all valid follow-ups but belong in a dedicated "bilingual error message" phase, not in a Gap 4 hotfix.

verification: "Not yet applied. After fix: run `uv run pytest src/mcp_canada/modules/british_columbia/__tests__/test_tools.py -x -v` (new regression test should go green), then re-run UAT Test 13 manually by calling `bc_get_water_wells(lang='fr')` with no filters and confirming `error.message` is French and `error.lang == 'fr'`."
files_changed: []
...
**Recommended treatment:** fix the single guard message under Gap 4 (minimum diff, unblocks Test 13), and file a separate follow-up item to audit all tool error paths for bilingual coverage. Do NOT roll the systemic fix into this gap — it would be a 50+ file change touching every module's tests and would balloon the phase 15 fix well beyond its remit.
