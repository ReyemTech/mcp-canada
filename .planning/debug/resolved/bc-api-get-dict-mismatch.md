...
resolved_by: "15-05 plan execution — _api_get rewritten in client.py:55-77"
---
...
fix: |
  Applied in plan 15-05 (commit 2125a92). Changed the 4 lines in
  british_columbia/client.py:55-78 to treat api_get's return as parsed JSON:
  ...
  Also removed `_make_http_response` MagicMock helper from test_client.py and
  added `TestSharedApiGetContract` class (3 tests) that would have caught the bug.

verification: |
  Confirmed: `uv run pytest tests/integration/ -v -m integration --timeout=120 -k Bc`
  → 11 passed, 230 deselected (0 failures, was 7 pre-fix).
